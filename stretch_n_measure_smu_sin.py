...
2) Change code so capacitance is read in place of resistance
3) Change AC measurement to a true AC instead of this hacky toggling shit we got going on
...
def sinusoid_maker(amplitude_strain, freq, offset_strain,delta_t):
    """
...
def main():
    ...
    try:
        ...
        for repeat in range(repeats):
            ...
            for step_indx in range(len(step_profile)):
                    ...
                    while ((pos_err > pos_err_tol) or (loop_time < max_loop_time)):
                        ...
                        # Read capacitance
                        t_s_cap = time.time()
                        current_cap, current_esr = cmeter350610.get_measurement(cmeter)
        ...
        # write_PosResForce_to_CSV(filename,res_data_o, res_data_i, avg_time_data_res, pos_data, time_data_pos,
        #     force_data, time_data_force)
        write_PosResForce_to_CSV(filename,cap_data, dissip_data, avg_time_data_cap, pos_data, time_data_pos,
            force_data, time_data_force)
        # zero the specimen
