...
def pos_outlier_corrector(strain_arr):
    ...
    DESCR: Smooths any erroneous zero values in data (i.e. strain measurement drops 
                                                      to zero erroneously aperiodically)
...
# def find_steady_states(t_arr, x_arr, n_diff_buf, d_tol):
#     """
#     DESCR: Determines when dx_dt is at a sufficiently low (d_tol) to be deemed at 
#     steady state and appends an index marker to a steady_states array
#     IN_PARAMS: time_arr, x_arr, n_diff_buf, d_tol
#     OUT: steady_states
#     NOTES: DOESN'T FLIPPIN' WORK
#     TODO:    
#     """
#     steady_states = []
#     diff_buf = np.zeros(n_diff_buf)
#     for i in range(len(x_arr)-n_diff_buf):
#         diff_buf = x_arr[i:n_diff_buf+i]
#         diff_sum = 0
#         for j in range(n_diff_buf-1):
#            diff_sum =  diff_sum + (x_arr[j+1]-x_arr[j])/(t_arr[j+1]-t_arr[j])
#         diff_avg = diff_sum/(n_diff_buf-1)
#         if abs(diff_avg) <= d_tol:
...
input_filename = "2_7-5_E4pin_20mm_v15_0.1_0.2_0.3_strain.csv"
